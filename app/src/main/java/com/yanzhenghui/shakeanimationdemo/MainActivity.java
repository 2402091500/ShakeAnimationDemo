package com.yanzhenghui.shakeanimationdemo;
...
public class MainActivity extends AppCompatActivity {
    ...
    private RadioGroup mRgChooseAnim;

    private boolean mUsingViewAnim = true;
    ...
    private void initViews() {
        mIvShake = (ImageView) findViewById(R.id.shake);
        mBtnStart = (AppCompatButton) findViewById(R.id.start);
        mRgChooseAnim = (RadioGroup) findViewById(R.id.choose_anim);
        ...
        mRgChooseAnim.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                if (checkedId == R.id.view_anim) {
                    mUsingViewAnim = true;
                } else {
                    mUsingViewAnim = false;
                }
            }
        });
    }
    ...
    private void startShakeAnim() {
        if (mUsingViewAnim) {
            startShakeByViewAnim(mIvShake, 2.5f, 1.0f, 1f, 500);
        } else {
            startShakeByPropertyAnim(mIvShake, 2.5f, 1.0f, 1f, 500);
        }
    }
